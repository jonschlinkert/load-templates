...
utils.generateKey = function generateKey(patterns, locals, opts) {
 ...
 * any nested `locals` properties, and any non-`utils.rootKeys`
 * properties.
 *
 * @param  {Object} `value`
...
utils.siftProps = function siftProps(value, locals) {
  ...
  opts = extend({}, opts, utils.pickOptions(locs));
  o.options = utils.flattenOptions(opts);
  o.locals = utils.flattenLocals(locs);
  return o;
};
